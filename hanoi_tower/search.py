...
def depth_first_search (problem: hanoi_states.ProblemHanoi, display: bool = False):
    """
    Realiza una búsqueda en profundidad para encontrar una solución a un problema de Hanoi. Y recuerda si ya
    paso por un estado e ignora seguir buscando en ese nodo para evitar recursividad.
    ...
    frontier = deque([tree_hanoi.NodeHanoi(problem.initial)])  # Creamos un stack con el nodo inicial

    explored = set()  # Este set nos permite ver si ya exploramos un estado para evitar repetir indefinidamente

    while frontier:
        node = frontier.pop() # Extraemos el ultimo nodo de la cola
